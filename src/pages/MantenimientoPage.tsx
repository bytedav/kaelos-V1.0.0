...
export default function MantenimientoPage({ onNavigateHome, onNavigateServicios }: MantenimientoPageProps) {
    ...
    <div className="w-full bg-[#fafbfe] text-slate-900 min-h-screen">
      ...
      <section className="relative w-full overflow-hidden bg-slate-900 min-h-[460px] md:min-h-[600px] flex items-center pt-24 pb-16 md:py-28 -mt-14 sm:-mt-16">
        <div 
          ...
          style={{ backgroundImage: `url('https://images.unsplash.com/photo-1563720223185-11003d516935?auto=format&fit=crop&w=1800&q=80')` }}
        />
            ...
            <div className="lg:col-span-7 text-left space-y-4 md:space-y-6 text-white drop-shadow-md">
              <div className="space-y-4">
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight leading-[1.1] text-white [text-shadow:_0_2px_12px_rgba(0,0,0,0.7)]">
                  Mantenimiento y taller especializado para tu moto
                </h1>
                <p className="text-sm sm:text-base font-medium text-slate-100 leading-relaxed max-w-2xl hidden md:block [text-shadow:_0_1px_8px_rgba(0,0,0,0.8)]">
                  Revisiones oficiales, neumáticos, frenos, transmisión y diagnosis electrónica con mecánicos certificados y piezas originales.
                </p>
              </div>

              <div className="space-y-3 pt-2 max-w-lg hidden md:block">
                {[
                  'Revisiones oficiales sin perder la garantía de marca',
                  'Recambios 100% homologados y primeras marcas',
                  'Presupuesto previo cerrado y sin sorpresas'
                ].map((text, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 text-center border-b border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          <div className="bg-[#f8f9fb]/60 border border-[#eef0f3] rounded-[24px] p-8 text-left space-y-4">
            ...
            <h3 className="text-[18px] sm:text-[20px] font-black text-slate-950 tracking-tight">Solicita tu Cita</h3>
            <p className="text-[13px] sm:text-[14px] font-semibold text-slate-500 leading-relaxed">Ponte en contacto indicándonos la revisión o avería a tratar.</p>
          </div>

          <div className="bg-[#f8f9fb]/60 border border-[#eef0f3] rounded-[24px] p-8 text-left space-y-4">
            <div className="text-[24px] sm:text-[26px] font-extrabold text-[#ffaab8] leading-none">02</div>
            <h3 className="text-[18px] sm:text-[20px] font-black text-slate-950 tracking-tight">Recepción en Taller</h3>
            <p className="text-[13px] sm:text-[14px] font-semibold text-slate-500 leading-relaxed">Dejas la moto en nuestras instalaciones y te facilitamos vehículo sustitutivo si lo requieres.</p>
          </div>

          <div className="bg-[#f8f9fb]/60 border border-[#eef0f3] rounded-[24px] p-8 text-left space-y-4">
            <div className="text-[24px] sm:text-[26px] font-extrabold text-[#ffaab8] leading-none">03</div>
            <h3 className="text-[18px] sm:text-[20px] font-black text-slate-950 tracking-tight">Moto Lista a Punto</h3>
            <p className="text-[13px] sm:text-[14px] font-semibold text-slate-500 leading-relaxed">Recibes tu moto revisada, probada en carretera y limpia.</p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center max-w-2xl mx-auto mb-12">
          <h2 className="text-2xl sm:text-3xl font-black text-slate-950 tracking-tight">Servicios de Taller</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { title: 'Revisión Periódica', desc: 'Mantenimiento preventivo según libro oficial del fabricante.', icon: Settings },
            { title: 'Cambio de Neumáticos', desc: 'Sustitución, equilibrado y montaje de primeras marcas (Michelin, Pirelli, Dunlop...).', icon: Wrench },
            { title: 'Frenos y Transmisión', desc: 'Pastillas, discos, kits de arrastre, correas de variador y embragues.', icon: ShieldCheck },
            { title: 'Diagnosis y Electricidad', desc: 'Escaneo de unidades ECU, detección de fallos de inyección y batería.', icon: Zap }
          ].map((item, idx) => {
